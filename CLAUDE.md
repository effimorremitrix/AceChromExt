...
- the manifest keeps `permissions: ["storage", "sidePanel"]`, CBP-only hosts,
  and a CSP with `script-src 'self'` + `connect-src 'none'`. `sidePanel` was
  added on 2026-09-22 and is the ACE and INTTRA helpers' only permission beyond
  storage: it buys a SURFACE and nothing else (no host, no network, no tab
  reading, no injection), and it REPLACED the action popup rather than adding
  to it - `default_popup` is gone from both manifests and the toolbar icon
  ...
  `docs/ARCHITECTURE.md`, "The one UI surface".
...
| Quickfill's portal toggle (Auto / ACE / INTTRA) | `MODES` in `quickfill-extension/src/ui/sidePanel.ts`, `FillMode` in `quickfill-extension/src/core/messages.ts`, the session key in `core/store.ts`. A pin only chooses which buttons are offered and which mapping table an UNNAMED screen is filled from (`ASSUMED_PAGE`, the create page, INTTRA only, named in the result line). It never widens a write: ambiguity is still refused, a missing row is still reported, nothing is still clicked. Auto still refuses an unnamed screen |
...
Seven things are built but not verified against the real system, and all must
...
7. The **surfaces changed on 2026-09-22 and not one of them has been opened on
   a live portal**. The operator reported that leaving an ACE or INTTRA screen
   ...
   reasons the split existed were properties of the action popup the compact
   surface used to be: a file picker does not close a side panel, and the
   operator sizes one themselves. It also ships fewer bytes, one bundle per
   helper instead of two. The tab strip WRAPS and the INTTRA container grid
   scrolls sideways, because ten screens now live somewhere the operator may
   leave at 300px. `docs/ARCHITECTURE.md`, "Why the wide panel went".

   What none of this changes: not one selector, not one mapping, not one
   refusal, and nothing in items 1 to 6. It changes where the buttons live and
   how long they stay, and that is all. A panel that stays open in front of a
   field the helper still cannot resolve is still a field the helper cannot
   ...
   which pins the whole ten-tab strip, the surface block in
