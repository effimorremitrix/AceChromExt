/**
 * The import template's data: its columns, the worked example, and the
 * column-by-column notes.
 *
 * One copy, read by two writers: scripts/generate-template.mjs (the committed
 * templates/ACE_Import_Template.xlsx) and scripts/playground.mjs (the example
 * workbook inside the Quickfill playground), so the example the operator
 * practises on is the example the template documents.
 *
 * The example is a real shape: a Californian almond shipment to Haifa, two
 * lines, a ZIM booking. Carrier is the SCAC, not the carrier's name, because
 * ACE Step 4 asks for "Carrier SCAC/IATA"; the container number passes its
 * ISO 6346 check digit, so the ACE Helper's block does not fire on it.
 */
...
export const INSTRUCTIONS = [
  ...
  ['Carrier / Vessel / BookingNumber', 'Transportation step values. Carrier is the 4-letter SCAC (ZIMU, MSCU), which is what ACE Step 4 asks for; a carrier name draws a warning.'],
